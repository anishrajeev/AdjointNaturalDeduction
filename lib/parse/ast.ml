...
let rec pp_exp (col : int) (e : exp) : string = match e with
  ...
  | Add (e1, e2) -> "(" ^ (pp_exp col e1) ^ " + " ^ (pp_exp col e2) ^ ")"
  | Minus (e1, e2) -> "(" ^ (pp_exp col e1) ^ " - " ^ (pp_exp col e2) ^ ")"
  | Div (e1, e2) -> "(" ^ (pp_exp col e1) ^ " / " ^ (pp_exp col e2) ^ ")"
  | Mult (e1, e2) -> "(" ^ (pp_exp col e1) ^ " * " ^ (pp_exp col e2) ^ ")"
  | Eq (e1, e2) -> "(" ^ (pp_exp col e1) ^ " = " ^ (pp_exp col e2) ^ ")"
...
let pp_defn defn = match defn with
